...
class Probe:
    ...
    def generate_single_probe(self,mrad,wavelength,gaussianVOA,preview=False):
        ...
        if mrad == 0:
            return backend.zeros((nx, ny), dtype=self.complex_dtype, device=self.device)+1
        ...
        if preview:
            ...
            ax.imshow(backend.fft.fftshift(reciprocal.T), cmap="inferno",extent=extent)
            ax.set_xlabel("kx ($\\AA^{-1}$)")
            ax.set_ylabel("ky ($\\AA^{-1}$)")
            plt.show()

        return backend.ifftshift(backend.ifft2(reciprocal))
    ...
    def defocus(self,dz): # POSITIVE DEFOCUS PUTS BEAM WAIST ABOVE SAMPLE, UNITS OF ANGSTROM
        if isinstance(dz,(int,float)):
            dz = backend.zeros(len(self._array))+dz
        kx_grid, ky_grid = backend.meshgrid(self.kxs, self.kys, indexing='ij')
    ...
    def addTemporalDecoherence(self,sigma_eV,N):
        ...
        self._array = backend.zeros((N,1,nx,ny))
    ...
    def addSpatialDecoherence(self,sigma_dz,N):
        ...
        self.eVs = backend.ones(N)[:,None]*self.eVs[None,:] # defocus expands into nz,nc then flattens to nz*nc
        self.eVs = self.eVs.reshape(nc)
        self.wavelengths = backend.ones(N)[:,None]*self.wavelengths[None,:]
    ...
    def applyShifts(self):
        ...
        if self.cropping:
            ...
            self._array = self._array[:,0,None,i1:i2,j1:j2] * backend.ones(len(self.probe_positions))[None,:,None,None]
        else:
            self._array = self._array[:,0,None,:,:] * backend.ones(len(self.probe_positions))[None,:,None,None]
    ...
    def aberrate(self,aberrations):
        ...
        reciprocal = backend.fft2(backend.fftshift(self._array))
        reciprocal *= dP
...
def aberrationFunction(kxs,kys,wavelength,aberrations): # aberrations should be a dict of Cnm following https://abtem.readthedocs.io/en/latest/user_guide/walkthrough/contrast_transfer_function.html
    dPhi = backend.zeros((len(kxs),len(kys)), dtype=backend.float_dtype, device=backend.device)
...
def Propagate(probe, potential, device=None, progress=False, onthefly=True, store_all_slices=False):
    ...
    probe_wavelengths = probe.wavelengths[:,None]*backend.ones(npt)[None,:] # also expand wavelengths and eVs arrays to cover all probe positions npt
    probe_wavelengths = probe_wavelengths.reshape(nc*npt)
    probe_eVs = probe.eVs[:,None]*backend.ones(npt)[None,:]
    ...
    for z in localtqdm(range(len(potential.zs))):
        ...
        if probe.cropping:
            t = backend.zeros( (len(sigma), probe.cropping, probe.cropping ), type_match=P)
            for p,o in enumerate(probe.offsets): # We want to go from i1,j2 to i1+cropping,j1+cropping, but sometimes i1 or j1 is negatuve
                pot = roll(potential_slice,-o[0],0)[:probe.cropping,:]
                pot = roll(pot,-o[1],1)[:,:probe.cropping]
                #pot = xp.roll(potential_slice,list(-o),(0,1))[:probe.cropping,:probe.cropping]
                t[p,:,:]=backend.exp(1j*sigma[p]*pot)
        else:
