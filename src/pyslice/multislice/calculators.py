...
from .sed import SED
from pyslice.backend import make_backend, to_cpu, to_numpy, NumpyBackend
...
class MultisliceCalculator:
    ...
    def run(self) -> WFData:
        ...
        if self.skip_vacuum and len(self.probe_positions) > 1 and self.aperture > 1 and self.min_dk:
            ...
            self.probe_indices = b.asarray(self.probe_indices)
        ...
        with tqdm(total=self.n_frames, desc="Processing frames", unit="frame") as pbar:
            for frame_idx in range(self.n_frames):
                ...
                cache_exists, frame_data = checkCache(cache_file, self.cache_levels, b)
                ...
                if cache_exists:
                    ...
                    for selected in chunks:
                        ...
                        exit_waves_single = Propagate(probe, potential, b, progress=show_progress, onthefly=True, store_all_slices=("slices" in self.cache_levels))  # [l],p,x,y indices
                        ...
                        for layer_idx in range(self.n_layers):
                            exit_waves_k = b.fft2(exit_waves_single[layer_idx, :, :, :], axes=(-2, -1))  # l,p,x,y --> p,x,y
...
def checkCache(cache_file, cache_levels, b):
    ...
    if cache_file.exists() and ("exitwaves" in cache_levels or "slices" in cache_levels):
        ...
        return True, b.asarray(np.load(cache_file))  # if always saving as numpy, then must cast to backend array if re-reading cache file back in
    return False, 0
