...
def infer_dimensions(dimensions_meta:Metadata|Dict, data_shape:Iterable[int], signal_type:Literal['2D-EELS','1D-EELS','Diffraction','Image']|None=None, small_angle:bool=True) -> str|None:
    ...
    if signal_type is None:
        if dimensions[-1]['space'] == 'spectral':
            ...
            signal_type = 'Image'
        elif dimensions_meta['sig_ndim']==2 and dimensions[-1]["units"] == "1/nm" and dimensions[-2]["units"] == "1/nm":
            signal_type = 'FFT'
        else:
            raise Warning('signal_type was not provided, and could not be inferred. The signal will be set as provided, but no signal dimension inference is provided.')
